...
class FilesAPIUser(HttpUser):
    """User class for the Files API."""

    wait_time = between(1, 5)

    @task
    def file_operations_flow(self):
        """Flow for file operations."""
        file_path = f"test_file_{str(uuid.uuid4())[:5]}.txt"
    ...
    def generate_ai_files_flow(self):
        """Flow for generating AI files."""
        # Generate a text file
        text_file_path = f"ai_text_{str(uuid.uuid4())[:5]}.txt"
        text_params = {"prompt": "Write a short poem about clouds", "file_type": "text"}
        self.client.post(
            f"/v1/files/generated/{text_file_path}",
            params=text_params,
            name="Generate AI Text",
        )

        # Generate an image file
        image_file_path = f"ai_image_{str(uuid.uuid4())[:5]}.png"
        image_params = {
            "prompt": "A beautiful landscape with mountains and a lake",
            "file_type": "image",
        }
        self.client.post(
            f"/v1/files/generated/{image_file_path}",
            params=image_params,
            name="Generate AI Image",
        )

        # Generate an audio file
        audio_file_path = f"ai_audio_{str(uuid.uuid4())[:5]}.mp3"
        audio_params = {
            "prompt": "Generate a short text-to-speech audio saying 'Welcome to our API'",
            "file_type": "text-to-speech",
        }
        self.client.post(
            f"/v1/files/generated/{audio_file_path}",
            params=audio_params,
