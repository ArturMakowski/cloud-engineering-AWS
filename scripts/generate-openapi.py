...
def parse_args() -> Args:
    ...
    Returns:
        Args: Parsed command-line arguments as a NamedTuple.
    """
...
def generate_openapi() -> dict:
    ...
    This function creates a FastAPI application instance and generates its OpenAPI schema
    by extracting metadata such as title, version, description, and routes from the app.
    ...
    Returns:
        dict: The complete OpenAPI schema containing all API metadata and endpoint definitions.

    Example:
        schema = generate_openapi()
        # Returns a dictionary containing the full OpenAPI specification
    """
...
def write_openapi_to_disk(openapi_schema: dict, outfile_path: Path) -> None:
    ...
    Args:
        openapi_schema (dict): The OpenAPI schema to write to disk.
        outfile_path (Path): The path to the output file.
    """
...
def get_diff_between_openapi_schemas(
    ...
    Args:
        input_spec (Path): The path to the input OpenAPI schema JSON file.
        generated_spec (dict): The generated OpenAPI schema.

    Returns:
        List[Diff]: A list of differences between the input and generated schemas.
    """
...
def diff_dicts(
    ...
    Args:
        ...
        path: The base path for nested keys.

    Yields:
        Generator yielding the differences as Diff objects.
    """
...
def diff_lists(list_a: list, list_b: list, path: str) -> Generator[Diff, None, None]:
    ...
    This function compares two lists element by element and yields their differences.
    For dictionary elements, it performs a deep comparison.

    Args:
        list_a (list): The first list to compare.
        list_b (list): The second list to compare.
        path (str): The base path for nested indices.

    Yields:
        Diff: A difference object containing:
            - path: Location of the difference
            - before: Value from list_a
            - after: Value from list_b

    Notes:
        - For elements that exist in list_a but not in list_b, 'after' will be None
        - For elements that exist in list_b but not in list_a, 'before' will be None
        - For dictionary elements, performs recursive comparison
    """
